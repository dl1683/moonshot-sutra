...
- **Cross-Tokenizer Likelihood Scoring (CTLS)** (Dec 2025, arXiv:2512.14954): Generalizes Phan et al. ICLR 2025 covering decomposition to arbitrary vocabulary pairs via "relative cover encodings." Key insight: BPE vocabularies form a nested chain V_0(bytes)→V_M(full vocab); any sub-vocabulary is a relative alphabet. For subset case (student vocab ⊆ teacher vocab), achieves O(1) model evaluations per token. Results: GSM8K cross-tokenizer KD achieves 55.6% (vs ALM 53.5%, DSKD 52.8%, ULD 48.2%). +2.1pp over SOTA. **Our covering is the M'=0 special case of their framework.** Single-teacher only. Validates our approach as theoretically grounded.
